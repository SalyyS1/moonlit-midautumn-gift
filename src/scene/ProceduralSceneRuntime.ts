...
export class ProceduralSceneRuntime {
  ...
  constructor(options: SceneRuntimeOptions) {
    ...
    queueMicrotask(() => {
      ...
      options.canvas.dataset.sceneReady = 'true';
      options.canvas.dispatchEvent(new CustomEvent('world-ready'));
    });
  }
  ...
  private render = () => {
    if (this.disposed || !this.visible) { this.raf = 0; return; }
    ...
    if (this.current >= .94 && !this.letterRevealed) {
      this.letterRevealed = true;
      this.renderer.domElement.dispatchEvent(new CustomEvent('letter-reveal', { detail: { progress: this.current, automatic: true } }));
    } else if (this.current < .82) this.letterRevealed = false;
    this.renderer.domElement.dispatchEvent(new CustomEvent('world-progress', { detail: {
      progress: this.current, chapterIndex: this.timeline.active(this.current), chapterId: this.timeline.id(this.current),
      beatId: this.timeline.beatId(this.current), beatProgress: this.timeline.beatProgress(this.current), ready: true,
    } }));
  ...
  private onVisibilityChange = () => {
    ...
    if (!this.visible) {
      ...
      this.raf = 0;
      this.lastTime = 0;
    } else if (!this.disposed && !this.reduced && !this.raf) {
      this.lastTime = performance.now();
      this.raf = requestAnimationFrame(this.render);
    }
  };
